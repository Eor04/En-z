...
export const OrderDriverRatingCard: React.FC<OrderDriverRatingCardProps> = ({
  ...
  return (
    <div className="p-6 rounded-3xl bg-slate-900/90 border border-amber-500/30 shadow-2xl space-y-4">
      ...
      {isSaved ? (
        <div className="p-4 rounded-2xl bg-slate-950/60 border border-slate-800 space-y-2 text-xs">
          ...
          {feedback && <p className="text-emerald-400 font-bold">{feedback}</p>}
        </div>
      ) : (
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="text-center py-2 bg-slate-950/50 rounded-2xl border border-slate-800 p-4">
            <span className="block text-xs font-bold text-slate-300 uppercase mb-2">
          ...
          <div>
            <textarea
              ...
              className="w-full px-3.5 py-2.5 rounded-xl bg-slate-950 border border-slate-800 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-amber-500/60 transition-all resize-none"
            />
          </div>
          ...
          <button
            ...
            className="w-full py-3 rounded-xl bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-400 hover:to-orange-400 text-slate-950 font-black text-xs shadow-lg shadow-amber-500/20 flex items-center justify-center gap-2 transition-all disabled:opacity-50"
          >
