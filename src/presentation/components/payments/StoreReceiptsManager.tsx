...
export function StoreReceiptsManager({
  ...
  return (
    <div className="space-y-6">
      ...
      ════════════════════════════════════════ */}
      <div className="p-5 rounded-3xl rune-panel border border-info/20 space-y-4">
        ...
        {qrFeedback && (
          <div className={`p-3 rounded-xl text-xs flex items-center gap-2 ${
            qrFeedback.startsWith('✅')
              ? 'bg-violet-950/40 border border-violet-500/40 text-violet-300'
              : 'bg-violet-950/40 border border-ember-deep text-ember-soft'
          }`}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-5 gap-4">
          ...
          <div className="sm:col-span-2 flex flex-col items-center justify-center p-3 rounded-2xl bg-white border border-surface-line text-center">
            {currentQrUrl ? (
              <>
                <img src={currentQrUrl} alt="Tu QR" className="w-28 h-28 object-contain mx-auto" />
                <span className="text-[10px] font-black uppercase mt-1 text-ink-faint">Tu QR Actual</span>
              </>
            ) : (
              <>
                <QrCode className="w-16 h-16 text-ink-soft mx-auto" />
                <span className="text-[10px] text-ink-mute mt-1">Sin QR configurado</span>
              </>
            )}
          </div>
          ...
          <div className="sm:col-span-3 space-y-2">
            <div className="flex items-center justify-between">
              <span className="text-[11px] text-ink-mute font-semibold">Imagen del código QR</span>
              <div className="flex gap-1">
                <button type="button" onClick={() => setQrTab('upload')}
                  className={`px-2 py-0.5 rounded-lg text-[10px] font-bold transition-all ${
                    qrTab === 'upload' ? 'bg-info-deep/20 text-info border border-info/40' : 'text-ink-faint hover:text-ink-soft'
                  }`}>
                  <Upload className="w-3 h-3 inline mr-0.5" />Subir
                </button>
                <button type="button" onClick={() => setQrTab('url')}
                  className={`px-2 py-0.5 rounded-lg text-[10px] font-bold transition-all ${
                    qrTab === 'url' ? 'bg-info-deep/20 text-info border border-info/40' : 'text-ink-faint hover:text-ink-soft'
                  }`}>
            ...
            {qrTab === 'upload' ? (
              <div
                ...
                className={`w-full rounded-2xl border-2 border-dashed cursor-pointer flex flex-col items-center justify-center gap-2 py-6 transition-all ${
                  qrDragging ? 'border-info bg-info/10' : 'border-surface-line hover:border-info-deep/50 hover:bg-surface-raised/30'
                } ${qrUploading ? 'pointer-events-none opacity-60' : ''}`}
              >
                {qrUploading ? (
                  <><Loader2 className="w-5 h-5 text-info animate-spin" /><span className="text-[11px] text-ink-mute">Subiendo QR...</span></>
                ) : (
              ...
              <div className="space-y-2">
                <input
                  ...
                  className="w-full p-2.5 rounded-xl bg-void-700 border border-surface-line text-white placeholder-ink-faint text-xs focus:border-info outline-none"
                />
                <button
                  type="button"
                  onClick={() => { if (qrUrlInput.trim()) saveQrUrl(qrUrlInput.trim()); }}
                  disabled={!qrUrlInput.trim() || qrUploading}
                  className="w-full py-2 rounded-xl bg-info-deep hover:bg-info text-white text-xs font-bold flex items-center justify-center gap-1.5 transition-all disabled:opacity-50"
                >
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-sm font-bold text-white uppercase tracking-wider flex items-center gap-2">
              <QrCode className="w-4 h-4 text-warn" />
          ...
          <button
            ...
            disabled={loading}
            className="p-2 rounded-xl bg-void-700 border border-surface-line hover:border-surface-line text-ink-soft text-xs flex items-center gap-1.5 transition-all"
          >
        ...
        {feedback && (
          <div className="p-3.5 rounded-2xl bg-violet-950/40 border border-violet-500/40 text-violet-300 text-xs flex items-center gap-2 animate-in fade-in">
        ...
        {loading ? (
          <div className="py-12 text-center text-ink-mute text-xs">
            <div className="w-6 h-6 border-2 border-warn border-t-transparent rounded-full animate-spin mx-auto mb-2" />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {receipts.map((payment) => {
              ...
              return (
                <div
                  key={payment.id}
                  className="p-5 rounded-3xl rune-panel border border-warn/30 hover:border-warn/50 transition-all space-y-4 shadow-xl shadow-violet-950/10"
                >
                  {/* Card Header */}
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-warn/20 text-warn-soft border border-warn/30">
                    ...
                    <div className="text-right">
                      <div className="text-[10px] text-ink-mute uppercase tracking-wider">Monto a Verificar</div>
                  ...
                  {/* Items Summary */}
                  <div className="p-2.5 rounded-xl bg-void-700/60 border border-surface-line text-[11px] text-ink-soft">
                    <div className="font-bold text-ink-mute mb-1 flex items-center gap-1">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between text-[11px]">
                      <span className="text-ink-mute font-semibold">Comprobante Adjunto:</span>
                      <a
                    ...
                    <div className="h-40 rounded-2xl overflow-hidden bg-void border border-surface-line relative group">
                      <img
                  ...
                  {isRejecting && (
                    <div className="p-3 rounded-2xl bg-violet-950/40 border border-ember-deep/80 space-y-2 text-xs">
                      <label className="block text-[11px] font-bold text-ember-soft">
                        Motivo del rechazo:
                      </label>
                      <input
                        type="text"
                        ...
                        className="w-full p-2 rounded-xl bg-void border border-ember-deep text-white text-xs outline-none focus:border-ember"
                      />
                      <div className="flex items-center gap-2 pt-1">
                        <button
                          ...
                          className="px-3 py-1.5 rounded-lg bg-ember-deep hover:bg-ember text-white text-xs font-bold"
                        >
                          Confirmar Rechazo
                        </button>
                        <button
                          ...
                          className="px-3 py-1.5 rounded-lg bg-surface-raised text-ink-soft text-xs"
                        >
                          Cancelar
                        </button>
                      </div>
                  ...
                  {/* Action Buttons */}
                  {!isRejecting && (
                    <div className="grid grid-cols-2 gap-2 pt-2 border-t border-surface-line/80">
                      <button
                        type="button"
                        disabled={isActing}
                        onClick={() => handleVerify(payment.id, true)}
                        className="py-2.5 px-3 rounded-xl bg-violet-600 hover:bg-violet-500 text-white text-xs font-bold shadow-md shadow-violet-600/20 flex items-center justify-center gap-1.5 transition-all disabled:opacity-50"
                      >
                      ...
                      <button
                        ...
                        className="py-2.5 px-3 rounded-xl bg-void-700 border border-surface-line hover:border-ember hover:text-ember text-ink-soft text-xs font-bold flex items-center justify-center gap-1.5 transition-all disabled:opacity-50"
                      >
