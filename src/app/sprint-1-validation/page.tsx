...
export default function Sprint1ValidationPage() {
  ...
  const getRoleColor = (role?: string) => {
    switch (role) {
      ...
      default:
        return 'text-ink-mute bg-surface-raised border-surface-line';
    }
  };
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <div className="relative rounded-3xl overflow-hidden rune-panel p-8 mb-8 border border-violet-500/20 shadow-2xl">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-6">
          <div>
            ...
            <p className="text-sm text-ink-soft max-w-2xl mt-2">
          ...
          <div className="flex items-center gap-3">
            ...
            >
              <RefreshCw className={`w-4 h-4 text-violet-400 ${loadingDb ? 'animate-spin' : ''}`} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
        ...
        <div className="rune-panel rounded-2xl p-6 border border-surface-line flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between mb-4">
              ...
              {session?.user ? (
                ...
                <span className="text-xs text-ink-faint">Sin sesión (Anónimo)</span>
              )}
            </div>

            {session?.user ? (
              <div className="space-y-3">
                <div className="p-3 rounded-xl bg-void-700/90 border border-surface-line">
                  <div className="text-[11px] text-ink-mute">Usuario Identificado</div>
                  <div className="font-bold text-white text-sm truncate">{session.user.name || 'Sin nombre'}</div>
                  <div className="text-xs text-ink-mute truncate">{session.user.email}</div>
                </div>

                <div className="p-3 rounded-xl bg-void-700/90 border border-surface-line">
                  <div className="text-[11px] text-ink-mute mb-1">Rol de Dominio Asignado</div>
                  <span
                ...
                {(session.user as any).driverCode && (
                  <div className="p-3 rounded-xl bg-void-700/90 border border-surface-line">
                    <div className="text-[11px] text-ink-mute">Código de Repartidor</div>
          ...
          {session?.user && (
            <button
              ...
              className="w-full mt-4 py-2 rounded-xl bg-void-700 hover:bg-violet-950/40 text-xs font-semibold text-ember-soft border border-surface-line hover:border-ember-deep/40 transition-colors"
            >
        ...
        <div className="rune-panel rounded-2xl p-6 border border-surface-line lg:col-span-2">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-sm font-bold text-white uppercase tracking-wider flex items-center gap-2">
              <Zap className="w-4 h-4 text-warn" />
              <span>Probar Roles del Sistema (1-Click Instant Login)</span>
            </h2>
            <span className="text-[11px] text-ink-mute font-medium">Validación Rápida</span>
          </div>
            ...
            <button
              ...
              className={`p-4 rounded-xl border text-left transition-all relative overflow-hidden group ${
                (session?.user as any)?.role === 'ADMIN'
                  ? 'bg-ember/10 border-ember/50 shadow-lg shadow-ember/10'
                  : 'bg-void-700/80 border-surface-line hover:border-ember/40 hover:bg-void-700'
              }`}
            >
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 rounded-xl bg-ember/20 text-ember flex items-center justify-center">
                    <ShieldCheck className="w-5 h-5" />
                  </div>
                  <div>
                    <div className="font-bold text-white text-xs">Administrador General</div>
                    <div className="text-[11px] text-ink-mute">admin@pedidostrinidad.com</div>
                  </div>
                </div>
                {(session?.user as any)?.role === 'ADMIN' && (
              ...
              <div className="mt-3 text-[11px] text-ink-mute line-clamp-1">
            ...
            <button
              ...
              className={`p-4 rounded-xl border text-left transition-all relative overflow-hidden group ${
                (session?.user as any)?.role === 'BUSINESS_OWNER'
                  ? 'bg-warn/10 border-warn/50 shadow-lg shadow-warn/10'
                  : 'bg-void-700/80 border-surface-line hover:border-warn/40 hover:bg-void-700'
              }`}
            >
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 rounded-xl bg-warn/20 text-warn flex items-center justify-center">
                    <Store className="w-5 h-5" />
                  </div>
                  <div>
                    <div className="font-bold text-white text-xs">Dueño de Tienda (Don Pepe)</div>
                    <div className="text-[11px] text-ink-mute">donpepe@elbosque.com</div>
                  </div>
                </div>
                {(session?.user as any)?.role === 'BUSINESS_OWNER' && (
            ...
            <button
              ...
              className={`p-4 rounded-xl border text-left transition-all relative overflow-hidden group ${
                (session?.user as any)?.role === 'DRIVER'
                  ...
                  : 'bg-void-700/80 border-surface-line hover:border-violet-500/40 hover:bg-void-700'
              }`}
            >
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="font-mono text-[11px] text-violet-400">Código: DRV-777</div>
                  </div>
                </div>
                {(session?.user as any)?.role === 'DRIVER' && (
            ...
            <button
              ...
              className={`p-4 rounded-xl border text-left transition-all relative overflow-hidden group ${
                (session?.user as any)?.role === 'CUSTOMER'
                  ? 'bg-info/10 border-info/50 shadow-lg shadow-info/10'
                  : 'bg-void-700/80 border-surface-line hover:border-info/40 hover:bg-void-700'
              }`}
            >
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 rounded-xl bg-info/20 text-info flex items-center justify-center">
                    <ShoppingBag className="w-5 h-5" />
                  </div>
                  <div>
                    <div className="font-bold text-white text-xs">Cliente (Mateo Morales)</div>
                    <div className="text-[11px] text-ink-mute">cliente@gmail.com</div>
                  </div>
                </div>
                {(session?.user as any)?.role === 'CUSTOMER' && (
      ...
      <div className="rune-panel rounded-2xl p-6 border border-surface-line">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
          ...
          {dbData && (
            <div className="flex flex-wrap items-center gap-2">
              <span className="px-3 py-1 rounded-lg bg-void-700 text-xs font-medium text-ink-soft border border-surface-line">
        ...
        {loadingDb ? (
          <div className="py-12 text-center text-ink-mute text-xs flex items-center justify-center gap-2">
          ...
          <div className="space-y-6">
            ...
            <div>
              <h3 className="text-xs font-bold text-ink-soft uppercase tracking-wider mb-3">
                Usuarios Registrados & Roles
              </h3>
              <div className="overflow-x-auto">
                <table className="w-full text-left text-xs border-collapse">
                  <thead>
                    <tr className="border-b border-surface-line text-ink-mute bg-void-700/50">
                  ...
                  <tbody className="divide-y divide-surface-line/60">
                    {dbData.users?.map((user: any) => (
                      <tr key={user.id} className="hover:bg-void-700/40 transition-colors">
                        <td className="py-2.5 px-3 font-medium text-white">{user.name || 'Sin Nombre'}</td>
                        <td className="py-2.5 px-3 text-ink-mute font-mono">{user.email}</td>
                        ...
                        <td className="py-2.5 px-3 font-mono text-violet-400">
                          {user.driverCode ? (
                            <span className="px-2 py-0.5 rounded bg-violet-500/10 border border-violet-500/30">
                              {user.driverCode}
                            </span>
                          ) : (
            ...
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                {dbData.spaces?.map((space: any) => (
                  <div
                    key={space.id}
                    className="p-3.5 rounded-xl bg-void-700/80 border border-surface-line flex flex-col justify-between"
                  >
                    <div>
                      <div className="font-bold text-white text-xs">{space.name}</div>
                      <p className="text-[11px] text-ink-mute mt-1 line-clamp-2">{space.description}</p>
                    </div>
                    <div className="mt-3 pt-2 border-t border-surface-line/80 text-[10px] text-violet-400 font-medium">
          ...
          <div className="p-4 rounded-xl bg-ember/10 border border-ember/20 text-ember-soft text-xs flex items-center gap-2">
